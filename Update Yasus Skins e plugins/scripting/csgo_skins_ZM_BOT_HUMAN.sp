...
// ====[ CONSTANTS ]=========================================================================
#define PLUGIN_NAME     "csgo_skins_ZM_BOT_HUMAN"
#define UPDATE_URL      "www.sourcemod.com"
...
{
	name        = PLUGIN_NAME,
	author      = "ROOT | AMAURI BUENO DOS SANTOS",
	description = "skin chooser for CS:GO Zombie Reload Humam CT",
...
public OnPluginStart()
{
	...
	CreateConVar("sm_csgo_skins_version", SOURCEMOD_VERSION, PLUGIN_NAME, FCVAR_NOTIFY|FCVAR_DONTRECORD);
	sc_enable     = CreateConVar("sm_csgo_skins_enable",  "1", "Whether or not enable CS:GO Skins Chooser plugin",                                   0, true, 0.0, true, 1.0);
	...
	sc_spawnmenu  = CreateConVar("sm_csgo_skins_spawnmenu",  "0", "Whether or not show the skins menu on player spawn", 							 0, true, 0.0, true, 1.0);
	sc_admflag    = CreateConVar("sm_csgo_skins_admflag", "",  "If flag is specified (a-z), only admins with that flag will able to use skins menu", 0);
...
public Action: PlayerDeathEvent(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//SetEntityModel(client,IsModelPrecached(client, "TSTE"));
	//SetEntityModel(client,ZR_GetHumanClass(client));
	//PrintToChatAll("amauri verificacao class DO ZOMBIE %s !", ZR_GetHumanClass(client));
	
	//PrintToChatAll("amauri verificacao class DO ZOMBIE %s !", ZR_GetZombieClass(client));
	
	if (ZR_IsClientZombie(client)) SetEntPropString(client, Prop_Send, "m_szArmsModel", "models/player/colateam/zombie1/arms.mdl");
	

	// And set the model
	//SetEntityModel(client,"models/player/XXX.mdl"); SKIN
	//SetEntPropString(client, Prop_Send, "m_szArmsModel", "models/weapons/eminem/arms_XXX.mdl"); ARMS MAO
...
public OnPlayerEvents(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	if (ZR_IsClientZombie(client)) return 0;
			...
			if (IsValidClient(client) && (random || !GetEntProp(client, Prop_Send, "m_bIsControllingBot")))
			{
				...
				switch (team)
				{
					...
					case CS_TEAM_CT: // Counter-Terrorists
					{
						...
						{
							// And set the model
							//SetEntityModel(client,"models/player/XXX.mdl");
							//SetEntPropString(client, Prop_Send, "m_szArmsModel", "models/weapons/eminem/arms_XXX.mdl");
